...
9. Notify Dave only for a verified completed delivery, a meaningful change in scope/risk, or a decision/access request. No idle polling commentary. Initial queued assignments without registered workers are awaiting initial launch, not implementation failures: keep the heartbeat active and quiet so later registrations are picked up. When all currently authorized work is complete or every remaining path requires a material decision/access authorization from Dave, report the remaining gates and pause the heartbeat rather than looping indefinitely. Dave's explicit stop or pause always overrides this waiting behavior.
